...
class ReceiverForm(forms.ModelForm):
    ...
    def __init__(self, *args, user_id=None, path_info=None, **kwargs):
        ...
        if (
            "receiver_update" in self.path_info
        ):  # if path is for update client, then email fields becomes inactive
            self.fields["email"].disabled = (
                True  # Make email field non-editable for editing client
            )
    ...
    def clean(self):
        ...
        if (
            user.adder.filter(email=email).exists()
            and "receiver_update" not in self.path_info
        ):
            self.add_error(
                "email",
                "You already have the client with inserted email. Please, either insert another email or edit client",
            )
...
class MessageForm(forms.ModelForm):
    ...
    def __init__(self, *args, user_id=None, path_info=None, **kwargs):
        ...
        if (
            "message_update" in self.path_info
        ):  # if path is for update message, then title field becomes inactive
            self.fields["title"].disabled = (
                True  # Make email field non-editable for editing message
            )
            self.fields["title"].help_text = (
                None  # Remove help_text for editing message
            )
    ...
    def clean(self):
        ...
        if (
            user.message_sender.filter(title=title).exists()
            and "message_update" not in self.path_info
        ):
            self.add_error(
                "title",
                "You already have such Title. Please, either insert another title or edit current message",
            )
...
class MailingForm(forms.ModelForm):
    ...
    def __init__(self, *args, path_info=None, **kwargs):
        ...
        self.fields["receivers"].queryset = Receiver.objects.filter(
            receiver_adder=self.user_id
        )
        self.fields["message"].widget = forms.RadioSelect()
        self.fields["message"].queryset = Message.objects.filter(
            message_sender=self.user_id
        )
    ...
    def clean(self):
        ...
        if (
            user.sender.filter(message__title=title).exists()
            and "mailings_update" not in self.path_info
        ):
            self.add_error(
                "message",
                "You already have Mailing with such title. Please, choose another mailing or edit current one",
            )
